...
export default class Aladino {
    constructor({
            canvas = document.createElement("canvas"),  // 새로운 캔버스에서 생성 //
            dpr = Math.min(devicePixelRatio, 2),        // 캔버스 크기 기본 2배로 //
            density = 1,    // 밀도가 높을수록 왜곡효과 상승 - 세그먼트 나누는 느낌
            autoScroll = true,  // 스크롤 반응여부
            post = false,
            attribs = { antialias: true,},
            autoTick = true,   // 계속 재생시킬 것인지 여부 - requestAnimationFrame 실행
        } = {}) {
        ...
        // 후처리 사용할 때 강제 안티앨리어싱 삭제 //
        if (this.post) attribs.antialias = false;
    ...
    init() {
        ...
        if (this.post) this.setupPost()
        if (this.autoTick) this.draw()
    }
    ...
    resize() {
        ...
        Object.assign(this.gl.canvas.style, { // --------------------------------------------- 캔버스 CSS
            position: "fixed",
            top: 0,
            right: 0,
            bottom: 0,
    ...
    draw(time = performance.now()) {
        if (this.autoTick) this.requestID = requestAnimationFrame(this.draw);
        ...
        if (this.post) gl.bindFramebuffer(gl.FRAMEBUFFER, this.postFb);

        gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);

        // TODO sort drawing by material?
        this.carpets.forEach((carpet) => {
            if (!carpet.active) return;
