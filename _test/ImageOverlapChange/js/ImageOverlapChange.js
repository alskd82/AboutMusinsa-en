...
class ImageOverlapChange {
    ...
    intervalActive(){
        this.next()
        if( typeof( this.opts.interval_Fn ) === 'function' ) this.opts.interval_Fn()
    }
    delayedCall(){
        this.killDelayedCall();
        this.interval = this.opts.intervalTime ? gsap.delayedCall( this.opts.intervalTime, this.intervalActive.bind(this) ) : undefined
    }
    killDelayedCall() {
...
const slider = new ImageOverlapChange({
    ...
    // interval_Fn: changeText_Fn, // 인터벌이 실행될 때 동작시킬 함수 넣기 
...
document.querySelector(".btn-stopInterval").addEventListener('click', (e)=>{
    // console.log( e.currentTarget.innerText )
    if(e.currentTarget.innerText === "intervalOff"){
        slider.killDelayedCall()
        e.currentTarget.innerText = "intervalOn";
    } else {
        slider.delayedCall()
        e.currentTarget.innerText = "intervalOff";
    }
})
