...
import { Mobile_Navi } from "./page/nav"
import { HomeInit, HomeST } from "./page/page_home"
import { Service } from "./page/page_service"
import { Partners_Mobile } from "./page/page_partners"
...
const load =()=>{
    ...
    if(nameSpace === "home"){
        ...
        ( async()=>{
            await Partners_Mobile.fetchPage()
            await Partners_Mobile.createBrand()

            let loaded = 0;
            for(let i=0; i<4; i++){
                const img = new Image()
                img.src = Partners_Mobile.getData().brandsImg[i];
                img.onload =()=>{
                    loaded ++;
                    if(loaded >= 4){
                        loadingComplete()
            ...
            StaggerMotion.st();
            // console.log( gsap.utils.toArray('[data-stagger-batch]').length )
...
const loadingComplete =()=> {
    ...
    if(document.body.classList.contains('before-load')){  //--------------------------------------------- 첫 실행
        loadDimAnimation( true )
    ...
    if(nameSpace != 'newsroom'){
        gsap.delayedCall( .85, function(){ // ------------------------------------------------------------ 텍스트 모션 시작
            BillboardText.play()
            setTimeout(() => {
                document.querySelector('.m_arrow-bottom') && (()=>{
                    document.querySelector(".m_arrow-bottom").classList.add('is-active')
                })()
            }, 700);
        }) 
    }
...
const pageBeforeEnter =()=>{
    ...
    Mobile_Navi.naviActive(nameSpace); // 현재 페이지 네비 강제활성화
